...
static uint8_t IsocInBuffDummy[48 * 8 * 2]; /* This dummy buffer with 0 values will be sent when there is no availble data */
static int16_t VOL_CUR;
static USBD_AUDIO_HandleTypeDef   haudioInstance;

volatile uint8_t USB_SETUP_DONE = 0;
...
void USBD_AUDIO_CDC_Init_Microphone_Descriptor(uint32_t samplingFrequency, uint8_t Channels)
{
  ...
  USBD_AUDIO_CDC_CfgDesc[index++] = 0x26 + Channels;       /*check*/         /* wTotalLength = 37+AUDIO_CHANNELS*/
  USBD_AUDIO_CDC_CfgDesc[index++] = 0x00;
                                 ...
                                 uint8_t cfgidx)
{
  ...
  ((USBD_AUDIO_CDC_ItfTypeDef *)pdev->pUserData)->Init_Audio(haudio->frequency, 0, haudio->channels);
                                   ...
                                   uint8_t cfgidx)
{
  ...
  {
    ((USBD_AUDIO_CDC_ItfTypeDef *)pdev->pUserData)->DeInit(0);
                                   ...
                                   uint8_t epnum)
{
  ...
  if (epnum == (AUDIO_IN_EP & 0x7F))
  {
    if (haudio->state == STATE_USB_IDLE)
    {
      ...
      ((USBD_AUDIO_CDC_ItfTypeDef *)pdev->pUserData)->Record();
    }
...
//                        length_usb_pck);
      ...
      {
        ((USBD_AUDIO_CDC_ItfTypeDef *)pdev->pUserData)->Stop();
        haudio->state = STATE_USB_IDLE;
        haudio->timeout = 0;
...
static uint8_t  USBD_AUDIO_EP0_RxReady (USBD_HandleTypeDef *pdev)
{
  ...
  if (haudio->control.cmd == AUDIO_REQ_SET_CUR)
  {
    ...
    {
      ((USBD_AUDIO_CDC_ItfTypeDef *)pdev->pUserData)->AudioCtl(haudio->control.cmd,
                               ...
                               uint8_t cfgidx)
{
  ...
  else
  {
    ...
    ((USBD_AUDIO_CDC_ItfTypeDef *)pdev->pUserData)->Init_CDC();
  ...
  switch (req->bmRequest & USB_REQ_TYPE_MASK)
  {
    case USB_REQ_TYPE_CLASS :
      if (req->wLength)
      {
        ...
        {
          ((USBD_AUDIO_CDC_ItfTypeDef *)pdev->pUserData)->CDCCtl(req->bRequest,
              (uint8_t *)hcdc->data,
              req->wLength);
          USBD_CtlSendData (pdev,
                            (uint8_t *)hcdc->data,
        ...
        if(req->bmRequest == 0x21 && req->bRequest == 0x20)
        {
          USB_SETUP_DONE = 1;
        }
      }
...
static uint8_t  USBD_CDC_DataOut (USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  if(pdev->pClassData != NULL)
  {
    ((USBD_AUDIO_CDC_ItfTypeDef *)pdev->pUserData)->Receive(hcdc->RxBuffer, &hcdc->RxLength);
    return USBD_OK;
  }
  else
  {
...
static uint8_t  USBD_CDC_EP0_RxReady (USBD_HandleTypeDef *pdev)
{
  ...
  if((pdev->pUserData != NULL) && (hcdc->CmdOpCode != 0xFF))
  {
    ...
    ((USBD_AUDIO_CDC_ItfTypeDef *)pdev->pUserData)->CDCCtl(hcdc->CmdOpCode,
        (uint8_t *)hcdc->data,
        hcdc->CmdLength);
    hcdc->CmdOpCode = 0xFF;
